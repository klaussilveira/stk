...
void bpress(XEvent *e) {
    ...
    for (ms = mshortcuts; ms < mshortcuts + LEN(mshortcuts);
    ms++) {
...
void selnotify(XEvent *e) {
    ...
    do {
        if (XGetWindowProperty(xw.dpy, xw.win, property, ofs, BUFSIZ / 4, False, AnyPropertyType, &type, &format,
                               &nitems,
                               &rem, &data)) {
...
void selrequest(XEvent *e) {
    ...
    if (xsre->target == xa_targets) {
        ...
        string = xsel.xtarget;
        XChangeProperty(xsre->display, xsre->requestor, xsre->property, XA_ATOM, 32, PropModeReplace, (uchar *) &string,
                        1);
        ...
        if (seltext != NULL) {
            XChangeProperty(xsre->display, xsre->requestor, xsre->property, xsre->target, 8, PropModeReplace,
...
void ximopen(Display *dpy) {
    ...
    xw.xic = XCreateIC(xw.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, xw.win,
                       XNFocusWindow,
                       xw.win, NULL);
    if (xw.xic == NULL)
...
void xinit(int cols, int rows) {
    ...
    xw.win = XCreateWindow(xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, XDefaultDepth(xw.dpy, xw.scr), InputOutput,
    ...
    if (IS_SET(MODE_FOCUSED)) {
        switch (win.cursor) {
            ...
            case 4: /* Steady Underline */
                XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + (cy + 1) * win.ch - cursorthickness,
                            win.cw,
                            cursorthickness);
                break;
            case 5: /* Blinking bar */
...
void kpress(XEvent *ev) {
    ...
    /* 1. shortcuts */
    for (bp = shortcuts; bp < shortcuts + LEN(shortcuts);
    bp++) {
